...
def add_text_to_cad(text_group:dict, p:APoint, padding = 4, text_height = 2.5):
    for text in text_group.values():
        text = acad.model.Addtext(text,p,text_height)
        p.y -= padding 
### Channels ###
def add_channel_info_text(channel: pd.Series, margin = 80, padding = 4, text_height = 2.5):
    ...
    add_text_to_cad(channel_infos,p,padding,text_height)
    ...
    if channel['max water depth']:
        ...
        text = acad.model.Addtext(max_flow,p,text_height)

def add_channel_vertex_text(channel: pd.Series, i: int, channel_name: str, last_one_flag: bool, padding = 4, text_height = 2.5):
    ...
    add_text_to_cad(channel_vertexs_texts,p,padding,text_height)
    if last_one_flag:
...
def add_pipes_text(pipes_table: pd.DataFrame):
    ...
    for pipe_name,pipe in pipes_table.iterrows():
        ...
        add_pipe_vertex_text(pipes_table, i, last_one_flag)
        add_pipe_info_text(pipe)         
        i += 1

def add_pipe_vertex_text(pipes_table: pd.DataFrame, i: int, last_one_flag: bool, padding = 4, text_height = 2.5):
    ...
    add_text_to_cad(pipe_vertexs_texts,p,padding,text_height)
...
def add_pipe_info_text(pipe: pd.Series, margin = 80, padding = 4, text_height = 2.5) :
    ...
    add_text_to_cad(pipe_info_text,p,padding,text_height)
...
def add_pump_info_text(pump: pd.Series,pump_name:str, margin = 30, padding = 4, text_height = 2.5) :
    ...
    add_text_to_cad(pump_info_text,p,padding,text_height)
