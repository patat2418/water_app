...
def create_pump(pipes_table: pd.DataFrame, pumps_table: pd.DataFrame)-> entities.Pump:
    ###  Pump data  ###
    pump_number = pumps_table['pump #'][0]
    pump_flow = pipes_table['consumption'].sum()  
    pumps_table['flow'][pump_number] = pump_flow
    return entities.Pump(rated_flow=pump_flow)

def calculate_flow_rate(pipes_table: pd.DataFrame, branches_df: pd.DataFrame, previous_pipe_number: int, pump:entities.Pump) -> tuple:
    if previous_pipe_number == 0:
        new_pipe_number = pipes_table[pipes_table['start with'] == 'Pump 1']['end with'].index[0]
        flow_rate = pump.rated_flow
    ...
    return new_pipe_number, flow_rate

def create_pipe(pipes_table: pd.DataFrame, new_pipe_number: int, flow_rate: float) -> entities.Pipe:
    ...
    return entities.Pipe(pipetype=pipetype, nominal_dia=nominal_dia, inside_dia=inside_dia,
                         length=length, static_head=static_head, flow_rate=flow_rate)

def update_pipe_data(pipes_table: pd.DataFrame, new_pipe_number: int, flow_rate: float, velocity: float,
                     head_loss: float, total_headloss: float):
...
def update_pump_data(pumps_table: pd.DataFrame, pump_head: float, pump_power: float, pump1: entities.Pump,
                     pump_flow: float, start_num: int):
    pump_number = pumps_table['pump #'][0]
...
def set_pressure_at_end_of_pipe(pipes_table: pd.DataFrame, pumps_table: pd.DataFrame, branches_df: pd.DataFrame):
    ...
    try:
        for i in range(1, len(pipes_table.index)):
            pipes_table.loc[next_pipe, 'Pressure at end of pipe'] = float(
                pipes_table.loc[current_pipe]['Pressure at end of pipe']) - float(
                pipes_table.loc[next_pipe, 'total head loss'])
...
def branched_network(pipes_table: pd.DataFrame,pumps_table: pd.DataFrame):
    ...
    ###  Pump data  ###
    pump_number = pumps_table['pump #'][0]
    pump = create_pump(pipes_table,pumps_table)
    ###  Pipes data  ###
    previous_pipe_number = 0

    for pipe in range(0,len(pipes_table.index)):

        new_pipe_number, flow_rate = calculate_flow_rate(pipes_table, branches_df, previous_pipe_number, pump)
        pipe1 = create_pipe(pipes_table, new_pipe_number, flow_rate)
        velocity = pipe1.velocity()
        head_loss = pipe1.major_head_loss()
        total_headloss = pipe1.total_head_loss()

        update_pipe_data(pipes_table, new_pipe_number, flow_rate, velocity, head_loss, total_headloss)
   ...
   ### Again some pumps data:    
    ...
    update_pump_data(pumps_table, pump_head, pump_power, pump, pump.rated_flow, start_num)
     
    set_pressure_at_end_of_pipe(pipes_table, pumps_table, branches_df)
